...
module ResourceMethods
  ...
  def render_object(object, serializer: nil, includes: true, jsonapi_included: nil)
    ...
    if serializer_used
      # Rails.logger.debug "***** USED SERializer #{filtered_serializer_includes(fields: fields)}"
      options = {
  ...
  def collection
    ...
    q = q.order(Arel.sql(order_string)) if order_string
  ...
  def generate_xls(serializer = nil, opts = {})
    ...
    opts[:serializer][:current_user] = current_user
    ...
    if params[:data]
      ...
      jsonapi_deserialize(
        params,
        {
          ...
          # NOTE: allowed params need to include relationship name if they are needed for save
